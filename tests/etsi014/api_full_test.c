...
static void test_key_exchange_protocol(void) {
    ...
    qkd_key_request_t request = {.number = 1,
                                 .size = 256,
                                 .additional_slave_SAE_IDs = NULL,
                                 .additional_SAE_count = 0};

    uint32_t result =
        GET_KEY(MKME_HOSTNAME, SLAVE_SAE, &request, &alice_container);
    if (result != QKD_STATUS_OK) {
    ...
    qkd_key_ids_t key_list = {
        .key_IDs = &id, .key_ID_count = 1, .key_IDs_extension = NULL};
    ...
    result =
        GET_KEY_WITH_IDS(SKME_HOSTNAME, MASTER_SAE, &key_list, &bob_container);
    if (result != QKD_STATUS_OK) {
